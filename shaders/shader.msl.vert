...
struct main0_out
{
    ...
    float3 out_normal [[user(locn2)]];
    float3 out_frag_pos [[user(locn3)]];
...
vertex main0_out main0(main0_in in [[stage_in]], constant UBO& _19 [[buffer(0)]])
{
    ...
    out.out_uv = in.uv;
    float4x4 _58 = transpose(spvInverse4x4(_19.model));
    out.out_normal = float3x3(_58[0].xyz, _58[1].xyz, _58[2].xyz) * in.normal;
    out.out_frag_pos = float3((_19.model * float4(in.position, 1.0)).xyz);
    return out;
}
